...
```bash
node "$REPO/bin/session-lens.mjs" -s <codex|pi|zcode|opencode|aionui> [选项]
```
...
node "$REPO/bin/session-lens.mjs" -s aionui -t RT-PRE-01 -r lead -n 6

# 2. 某项目下 pi 的最近会话
node "$REPO/bin/session-lens.mjs" -s pi --cwd enki-next-v9 -n 3

# 3. codex/zcode/opencode 最近会话列表，再挑一个看详情
node "$REPO/bin/session-lens.mjs" -s codex -l
node "$REPO/bin/session-lens.mjs" -s zcode -t 关键字 -n 4
node "$REPO/bin/session-lens.mjs" -s opencode -i ses_xxx --json
```
